...
const WorkspaceCard = ({ workspace, onInitiateChat }) => {
  ...
  return (
    <div className="bg-white/95 backdrop-blur-2xl shadow-2xl rounded-[2.5rem] pt-16 pb-8 px-8 hover:scale-[1.04] hover:shadow-purple-300/40 transition-all duration-300 border border-purple-100 flex flex-col gap-3 relative overflow-visible group" style={{ minWidth: 320, maxWidth: 370 }}>
      ...
      <div className="mt-6 text-center">
        <h3 className="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-700 via-purple-700 to-pink-600 drop-shadow-lg tracking-tight mb-1">{workspace.name}</h3>
        <p className="text-gray-500 text-sm mb-2">ID: <span className="font-bold text-purple-700 underline underline-offset-2 decoration-purple-300">{workspace.human_id}</span></p>
        <span
        ...
        <p className="text-gray-400 text-xs mt-3">Joined: {workspace.joined_at ? new Date(workspace.joined_at).toLocaleDateString() : '-'}</p>
      </div>
      <div className="mt-8 flex flex-col gap-3">
        {workspace.role === 'owner' && (
          <button
            ...
            className={`flex items-center justify-center gap-2 px-5 py-2 rounded-xl text-white font-bold transition-all duration-200 shadow-lg ${
              chatInitiated ? 'bg-gray-300 cursor-not-allowed text-gray-500' : 'bg-gradient-to-r from-blue-500 to-purple-500 hover:scale-105 hover:bg-blue-700'
        ...
        <button
          ...
          className={`flex items-center justify-center gap-2 px-5 py-2 rounded-xl text-white font-bold transition-all duration-200 shadow-lg ${
            workspace.role === 'owner' ? 'bg-gradient-to-r from-green-400 to-green-600 hover:scale-105 hover:bg-green-700' : 'bg-gradient-to-r from-blue-500 to-purple-500 hover:scale-105 hover:bg-blue-700'
          } group-hover:ring-2 group-hover:ring-purple-400`}
        >
        ...
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 7v4a1 1 0 001 1h3m10-5v4a1 1 0 01-1 1h-3m-4 4h4m-2 0v4m0-4V7" /></svg>
          Go to Boards
        </button>
      </div>
    </div>
